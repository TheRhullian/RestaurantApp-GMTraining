...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(createGpsSheet),
                                               name: NSNotification.Name(rawValue: "goButton"),
                                               object: nil)
    }
    ...
    func getRestaurantPlaces(coordinate:CLLocationCoordinate2D) {
        ...
        dataProvider.fetchPlacesNearCoordinate(coordinate, radius: searchRadius, types: ["restaurantes"]) { places in
            print(coordinate)
            for place in places {
